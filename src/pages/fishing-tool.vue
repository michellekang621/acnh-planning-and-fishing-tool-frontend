  ...
  <div>
    <div class="split left">
      <div class="card-container">
        <h1 class="card-title">Fishing Tool</h1>
        <md-card class="card">
          <p class="instructions-text"><i>Input your time and location to see the fish that you can currently catch!</i></p>
        ...
        <div class="split right">
          <div v-if="fishResults" class="search-results">
            <div v-for="fish in fishResults" :key="fish._id">
              <md-card class="search-card-container">
                <md-card-header>
                  <div class="search-card-content">
                    <div class="search-card-content-text">
                      <p>Name: {{ fish.item.name["name-USen"] }}</p>
                      <p>Sell Price: {{ fish.item.price }}</p>
                      <p>Location: {{ fish.item.availability.location }}</p>
                    </div>
                    <img
                      @click="
                        $router.push({
                          name: 'item-details',
                          params: { itemId: fish._id },
                        })
                      "
                      class="img-sizer"
                      :src="fish.item.image_uri"
                    />
                  </div>
                </md-card-header>
              </md-card>
...
export default {
  ...
  methods: {
    searchFish() {
      itemsDb
        .findAvailableFish(this.hemisphere, this.month, this.time)
        .then((fish) => {
...
.left {
  left: 0;
  background-image: url("../assets/pattern-bgs/acnh-leaf-light-blue.png");
