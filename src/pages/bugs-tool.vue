  ...
  <div>
    <div class="split left">
      <div class="card-container">
        <h1 class="card-title">Bugs Tool</h1>
        <md-card class="card">
          <p class="instructions-text"><i>Input your time and location to see the bugs that you can currently catch!</i></p>
          ...
          <md-button @click="searchBugs()"> Search! </md-button>
        </md-card>

        <div class="split right">
          <div v-if="bugResults" class="search-results">
            <div v-for="bug in bugResults" :key="bug._id">
              <md-card class="search-card-container">
                <div class="search-card-content">
                  <div class="search-card-content-text">
                    <p>Name: {{ bug.item.name["name-USen"] }}</p>
                    <p>Sell Price: {{ bug.item.price }}</p>
                    <p>Location: {{ bug.item.availability.location }}</p>
                  </div>
                  <img
                    @click="
                      $router.push({
                        name: 'item-details',
                        params: { itemId: bug._id },
                      })
                    "
                    class="img-sizer"
                    :src="bug.item.image_uri"
                  />
                </div>
              </md-card>
            </div>
...
export default {
  ...
  methods: {
    searchBugs() {
      itemsDb
        .findAvailableBugs(this.hemisphere, this.month, this.time)
        .then((bugs) => {
          let unsorted = bugs.data.map((bug) => bug.item.price);
          console.log(unsorted);
          bugs.data.sort(function (bug1, bug2) {
            return bug1.item.price - bug2.item.price;
          });
          let sorted = bugs.data.map((bug) => bug.item.price);
          console.log(sorted);
          this.bugResults = bugs.data;
          console.log(this.bugResults);
        });
    },
  },
};
...
.left {
  left: 0;
  background-image: url("../assets/pattern-bgs/acnh-leaf-light-green.png");
